function [connectivity_matrix, intersection_node_indices] = ...
    extract_cartpath_connectivity(parsed_osm)
...
for curway=1:ways_num
    ...
    if strcmp(key, 'golf') == 0 || strcmp(val, 'cartpath') == 0
        continue;
    else
        %disp('Found cartpath way');
        %disp(way.id(curway));
    end
    ...
    % inspect nodeset
    if way.id(curway) == 772145420
        disp('way 772145420 nodeset:');
        disp(nodeset);
        disp('way 772145420 nodeset size:');
        disp(nodes_num);
    end
